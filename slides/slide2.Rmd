...
---
#### Documenter votre fonction  
# Aller plus vite avec `{prefixer}`  
.pull-left[
Le package `{prefixer}` permet de finaliser votre fonction et produire la documentation plus facilement.  
![](www/addin_prefixer_logo.png)  
Il s'installe via :   
```{r eval = FALSE}
remotes::install_github("dreamRs/prefixer")
```
Il s'agit d'un package qui installe un addins, c'est à dire qui ajoute des fonctionnalités à RStudio.
...
---
#### Documenter votre fonction  
# `{prefixer}` : commande `Prefixer::`  
La commande `Prefixer::` scanne votre fonction, ouvre une boite de dialogue qui vous propose d'ajouter, pour chaque fonction utilisée dans notre script de définition de fonction, le préfixe adéquat.    
...
---
#### Documenter votre fonction  
# `{prefixer}` : commande `@importFrom`  
La commande `@importFrom` scanne votre script de définition de fonction, détecte toutes les préfixes utilisés et ajoute au dessus la ou les balises @importFrom package fonctions
...
---
#### Documenter votre fonction  
# Aller plus vite avec `{prefixer}` : commande `Not-ASCII`  

Utiliser un encodage multi-plateforme est absolument nécessaire pour que notre package puisse être installé et fonctionner partout, que ce soit sur un serveur linux, un PC, un Mac... 

La commande `@Not-ASCII` scanne votre script de définition de fonction, détecte toutes les caractères qui peuvent poser problème dans le corps du script (dans les chaînes de caractères entre "") et les convertit avec leur code.
Par exemple 
...
"Source : enqu\u00eate sur le parc locatif social, MTES" 
```
]  
...
Ca ne fonctionne pas dans les commentaires de documentation, de toutes façons, ça ne serait rendu correctement.
