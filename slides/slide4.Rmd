...
Il rassemble la documentation du package sous forme de site web.
...
- `usethis::use_pkgdown()` pour le construire   
- `pkgdown::build_site()` pour le visualiser  

to do : visualiser le site d'un package installé
...
Les instructions sont définies dans un fichier YAML, par exemple `.gitlab-ci.yml` pour gitlab, voire dans un dossier de fichiers YAML pour Github `.github`.
---
#### Publier notre site grâce avec la CI 
## Exemples
...
un exemple basique pour un package : https://gitlab.com/dreal-datalab/bilan.eco.insee/-/blob/master/.gitlab-ci.yml

### Github  

usethis plus fourni pour Github : une dizaine de fonctions dédiées à l'intégration continue sur Github.  

`usethis::use_github_actions` -> la plus générique pour un projet R  
